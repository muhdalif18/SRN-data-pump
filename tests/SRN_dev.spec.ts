...
test("test", async ({ page }) => {
  ...
  await page
    .getByText("Duti Setem (DEV) e-Duti")
    .waitFor({ state: "visible", timeout: 10000 });
  await page.getByText("Duti Setem (DEV) e-Duti").click();
  ...
  const progressFile = "./test-data/progress-SRN_dev.txt";
  ...
  for (let i = startIteration; i <= 40; i++) {
    ...
    await page.locator("#Input_UsernameVal").fill("userstds31@hasil.gov.my");
    await page.locator("#Input_UsernameVal").click();
    await page.locator("#Input_PasswordVal").click();
    await page.locator("#Input_PasswordVal").fill("990101019031");
