...
test("test", async ({ page }) => {
  ...
  await page.getByText("NORLELA BINTI CHE RUS").click();
  ...
  const progressFile = "./test-data/progress-SRN-pump-peranan.txt";
  ...
  for (let i = startIteration; i <= 40; i++) {
    ...
    await page
      ...
      .fill(`${docTitlePrefix}Form of Transfer of Securites${i}`);
    await page
    ...
    await page.locator("#StampingForm_formC_2_ShrTot").fill("132,3211");
    await page.locator("#StampingForm_formC_2_ShrTot").press("Tab");
    await page.locator("#StampingForm_formC_2_ShrTrf").fill("132,1321");
    await page.getByRole("radio", { name: "Ringgit Malaysia" }).check();
    await page.getByRole("textbox", { name: "0.00" }).click();
    await page.getByRole("textbox", { name: "0.00" }).fill("4,565,464.565");
    ...
    await page
      ...
      .click();
    await page.locator('input#wakilDiri[type="radio"]').check();
    await page.waitForTimeout(3000);
    await page.locator('input#PihakPertama[type="radio"]').check();
    await page.getByRole("button", { name: "Hantar " }).click();
