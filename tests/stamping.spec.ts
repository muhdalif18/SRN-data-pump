...
test("test", async ({ page }) => {
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    const randomAmount = parseInt(`${firstDigit}${firstDigit}${firstDigit}${remainingDigits.toString().substring(1)}`);
    const formattedAmount = randomAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
